...
(function (win, doc, undefined) {
            ...
            obj.canvas.rgraph_mouseup_event_listener = function (e) {
                ...
                if (objects) {
                    for (var i = 0, len = objects.length; i < len; i += 1) {
                        var obj = objects[i], id = objects[i].id;
            ...
            obj.canvas.rgraph_mousemove_event_listener = function (e) {
                ...
                if (objects && objects.length > 0) {
                    for (var i = 0, len = objects.length; i < len; i += 1) {
                        ...
                        if (shape && (tooltips && tooltips[shape['index']] || shape['tooltip']) && tooltips_event.indexOf('mousemove') !== -1 && (RG.isNull(current_tooltip) || obj.uid != current_tooltip.__object__.uid || (current_tooltip.__index__ != shape['index']) || (typeof shape['dataset'] === 'number' && shape['dataset'] != current_tooltip.__shape__['dataset']))) {
                            RG.clear(obj.canvas);
                            RG.hideTooltip();
                            RG.redraw();
    ...
    RG.drawCrosshairs = RG.DrawCrosshairs = function (e, obj) {
        var e = RG.fixEventObject(e), width = obj.canvas.width, height = obj.canvas.height, mouseXY = RG.getMouseXY(e),
            x = mouseXY[0], y = mouseXY[1], gutterLeft = obj.gutterLeft, gutterRight = obj.gutterRight,
            gutterTop = obj.gutterTop, gutterBottom = obj.gutterBottom, Mathround = Math.round, prop = obj.properties,
            co = obj.context, ca = obj.canvas
        RG.redrawCanvas(ca);
