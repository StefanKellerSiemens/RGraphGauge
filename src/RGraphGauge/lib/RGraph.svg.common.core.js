...
(function (win, doc, undefined) {
    ...
    RG.SVG.createDefs = function (obj) {
        var defs = RG.SVG.create({svg: obj.svg, type: 'defs'});
        obj.defs = defs;
        return defs;
    };
    RG.SVG.create = function (opt) {
        var ns = "http://www.w3.org/2000/svg", tag = doc.createElementNS(ns, opt.type);
    ...
    RG.SVG.drawXAxis = function (obj) {
        ...
        if (prop.xaxis) {
            ...
            var axis = RG.SVG.create({
                ...
                type: 'path',
                attr: {
                    d: 'M{1} {2} L{3} {4}'.format(prop.gutterLeft, y + 0.01, obj.width - prop.gutterRight, y),
            ...
            if (obj.type === 'hbar') {
                var width = obj.graphWidth / obj.data.length, x = prop.gutterLeft,
            ...
            if (prop.xaxisTickmarks) {
                if (prop.xaxisScale) {
                    ...
                    if (prop.xaxisLabelsPosition === 'section') {
                        ...
                        for (var i = 0; i < len; ++i) {
                            var gap = ((obj.graphWidth) / (len - 1)), x = prop.gutterLeft + ((i + 1) * gap);
                            RG.SVG.create({
                                svg: obj.svg,
                                type: 'path',
                                attr: {
                ...
                if (prop.yaxis === false) {
                    RG.SVG.create({
                        ...
                        type: 'path',
                        attr: {
                            d: 'M{1} {2} L{3} {4}'.format(prop.gutterLeft + 0.001, startY, prop.gutterLeft, endY),
    ...
    RG.SVG.drawYAxis = function (obj) {
        ...
        if (prop.yaxis) {
            ...
            var axis = RG.SVG.create({
                ...
                type: 'path',
                attr: {
                    d: 'M{1} {2} L{3} {4}'.format(x, prop.gutterTop, x + 0.001, obj.height - prop.gutterBottom),
            ...
            if (obj.type === 'hbar') {
                var height = obj.graphHeight / prop.yaxisLabels.length, y = prop.gutterTop,
                ...
                if (prop.yaxisTickmarks) {
                    ...
                    if (prop.xaxis === false) {
                        var axis = RG.SVG.create({
                            ...
                            type: 'path',
                            attr: {
                                d: 'M{1} {2} L{3} {4}'.format(obj.getXCoord(0) - prop.yaxisTickmarksLength - 1, obj.height - prop.gutterBottom, obj.getXCoord(0) + (obj.type === 'hbar' && prop.xaxisMin < 0 ? 3 : 0), obj.height - prop.gutterBottom - 0.001),
                ...
                var height = obj.graphHeight / prop.yaxisLabelsCount, y = prop.gutterTop, len = prop.yaxisLabelsCount,
                    startX = prop.gutterLeft, endX = prop.gutterLeft - prop.yaxisTickmarksLength;
                if (prop.yaxisTickmarks) {
                    for (var i = 0; i < len; ++i) {
                        var axis = RG.SVG.create({
                            svg: obj.svg,
                    ...
                    if (obj.type !== 'hbar' && (prop.yaxisMin !== 0 || prop.xaxis === false)) {
                        var axis = RG.SVG.create({
                            ...
                            type: 'path',
                            attr: {
                                d: 'M{1} {2} L{3} {4}'.format(prop.gutterLeft - prop.yaxisTickmarksLength, obj.height - prop.gutterBottom, prop.gutterLeft, obj.height - prop.gutterBottom - 0.001),
                                stroke: prop.yaxisColor,
                                'shape-rendering': "crispEdges"
                            }
                        });
        ...
        if (prop.yaxisScale) {
            ...
            for (var i = 0; i < prop.yaxisLabels.length; ++i) {
                ...
                var text = RG.SVG.text({
                    ...
                    x: prop.gutterLeft - 7 + prop.yaxisLabelsOffsetx,
                    y: y,
                    halign: 'right',
                    valign: 'center',
                    font: prop.yaxisTextFont || prop.textFont,
        ...
        if (prop.backgroundGrid) {
            ...
            if (prop.backgroundGridHlines) {
                ...
                for (var i = 0; i < count; ++i) {
                    parts.push('M{1} {2} L{3} {4}'.format(prop.gutterLeft, prop.gutterTop + (obj.graphHeight / count) * i, obj.width - prop.gutterRight, prop.gutterTop + (obj.graphHeight / count) * i));
                }
            ...
            if (prop.backgroundGridVlines) {
                ...
                for (var i = 0; i <= count; ++i) {
                    parts.push('M{1} {2} L{3} {4}'.format(prop.gutterLeft + ((obj.graphWidth / count) * i), prop.gutterTop, prop.gutterLeft + ((obj.graphWidth / count) * i), obj.height - prop.gutterBottom));
                }
            }
            if (prop.backgroundGridBorder) {
                parts.push('M{1} {2} L{3} {4} L{5} {6} L{7} {8} z'.format(prop.gutterLeft, prop.gutterTop, obj.width - prop.gutterRight, prop.gutterTop, obj.width - prop.gutterRight, obj.height - prop.gutterBottom, prop.gutterLeft, obj.height - prop.gutterBottom));
            }
    ...
    RG.SVG.getScale = function (opt) {
        var obj = opt.object, prop = obj.properties, numlabels = opt.numlabels, unitsPre = opt.unitsPre,
            unitsPost = opt.unitsPost, max = Number(opt.max), min = Number(opt.min), strict = opt.strict,
            decimals = Number(opt.decimals), point = opt.point, thousand = opt.thousand, originalMax = max,
            round = opt.round, scale = {max: 1, labels: [], values: []}, formatter = opt.formatter;
        if (!max) {
            var max = 1;
    ...
    RG.SVG.arrayFill = RG.SVG.arrayPad = function (opt) {
        var arr = opt.array, len = opt.length, value = (opt.value ? opt.value : null);
        if (arr.length < len) {
    ...
    RG.SVG.numberFormat = function (opt) {
        var obj = opt.object, prepend = opt.prepend ? String(opt.prepend) : '',
            append = opt.append ? String(opt.append) : '', output = '',
            decimal_seperator = typeof opt.point === 'string' ? opt.point : '.',
            thousand_seperator = typeof opt.thousand === 'string' ? opt.thousand : ',', num = opt.num;
        RegExp.$1 = '';
    ...
    RG.SVG.text = function (opt) {
        var obj = opt.object, parent = opt.parent, size = opt.size, bold = opt.bold, font = opt.font,
            italic = opt.italic, halign = opt.halign, valign = opt.valign, str = opt.text, x = opt.x, y = opt.y,
            color = opt.color ? opt.color : 'black', background = opt.background || null, padding = opt.padding || 0;
        if (halign === 'right') {
            halign = 'end';
        ...
        if (typeof background === 'string') {
            var bbox = text.getBBox(), rect = RG.SVG.create({
                svg: obj.svg,
                type: 'rect',
                attr: {
                    x: bbox.x - padding,
    ...
    RG.SVG.setShadow = function (options) {
        var obj = options.object, offsetx = options.offsetx || 0, offsety = options.offsety || 0,
            blur = options.blur || 0, opacity = options.opacity || 0, id = options.id;
        var filter = RG.SVG.create({
            svg: obj.svg,
            parent: obj.defs,
            type: 'filter',
            attr: {id: id, width: "130%", height: "130%"}
        });
        RG.SVG.create({
            svg: obj.svg,
            parent: filter,
            type: 'feOffset',
            attr: {result: 'offOut', 'in': 'SourceGraphic', dx: offsetx, dy: offsety}
        });
        RG.SVG.create({
            svg: obj.svg,
            parent: filter,
        ...
        RG.SVG.create({
            ...
            parent: filter,
            type: 'feGaussianBlur',
            attr: {result: 'blurOut', 'in': 'matrixOut', stdDeviation: blur}
        });
        RG.SVG.create({
            svg: obj.svg,
            parent: filter,
    ...
    RG.SVG.TRIG.getArcPath = function (options) {
        options.start -= 1.57;
        options.end -= 1.57;
        var start = RG.SVG.TRIG.toCartesian({cx: options.cx, cy: options.cy, r: options.r, angle: options.start});
        var end = RG.SVG.TRIG.toCartesian({cx: options.cx, cy: options.cy, r: options.r, angle: options.end});
    ...
    RG.SVG.TRIG.getRadiusEndPoint = function (opt) {
        if (arguments.length === 1) {
            var angle = opt.angle, r = opt.r;
        } else if (arguments.length === 4) {
            var angle = arguments[0], r = arguments[1];
        }
        var x = ma.cos(angle) * r, y = ma.sin(angle) * r;
        return [x, y];
    };
    ...
    RG.SVG.parseColorLinear = function (opt) {
        var obj = opt.object, color = opt.color;
        if (!color || typeof color !== 'string') {
            return color;
        }
        if (color.match(/^gradient\((.*)\)$/i)) {
            var parts = RegExp.$1.split(':'), diff = 1 / (parts.length - 1);
            ...
            var stop = RG.SVG.create({
                type: 'stop',
                parent: grad,
                attr: {offset: '0%', 'stop-color': RG.SVG.trim(parts[0])}
            });
            for (var j = 1, len = parts.length; j < len; ++j) {
                RG.SVG.create({
                    type: 'stop',
                    parent: grad,
                    attr: {offset: (j * diff * 100) + '%', 'stop-color': RG.SVG.trim(parts[j])}
                });
            }
        }
    ...
    RG.SVG.addCustomEventListener = function (obj, name, func) {
        ...
        RG.SVG.events[obj.uid].push({object: obj, event: name, func: func});
        return RG.SVG.events[obj.uid].length - 1;
    };
    RG.SVG.fireCustomEvent = function (obj, name) {
        if (obj && obj.isRGraph) {
            var uid = obj.uid;
            if (typeof uid === 'string' && typeof RG.SVG.events === 'object' && typeof RG.SVG.events[uid] === 'object' && RG.SVG.events[uid].length > 0) {
    ...
    RG.SVG.removeCustomEventListener = function (obj, i) {
        if (typeof RG.SVG.events === 'object' && typeof RG.SVG.events[obj.uid] === 'object' && typeof RG.SVG.events[obj.uid][i] === 'object') {
    ...
    RG.SVG.measureText = function (opt) {
        var text = opt.text || '', bold = opt.bold || false, font = opt.font || 'Arial', size = opt.size || 10,
    ...
    RG.SVG.getAdjustedNumber = function (opt) {
        var value = opt.value, prop = opt.prop;
    ...
    RG.SVG.attribution = function (obj) {
        ...
        var a;
        var x = parseFloat(obj.svg.getAttribute('width')) - 2, y = parseFloat(obj.svg.getAttribute('height')) - 2;
